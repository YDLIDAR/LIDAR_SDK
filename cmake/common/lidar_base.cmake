...
#			list: a b c
#
function(lidar_parse_function_args)
...
macro( add_to_lidar_include_dirs )
foreach( dir ${ARGN} )
  set_property( GLOBAL APPEND PROPERTY LIDAR_INCLUDE_DIRS "${dir}" )
  endforeach()
endmacro()


macro( add_to_lidar_libraries )
    foreach( lib ${ARGN} )
      ...
      if (TARGET ${lib})
          ...
          if (NOT ${is_lib_imported})
            set_property( GLOBAL APPEND PROPERTY LIDAR_LIBRARIES "${lib}" )
          else()
            # For imported targets, we just want to depend on the library directly
            get_target_property(libpath ${lib} LOCATION)
            if (libpath)
                set_property( GLOBAL APPEND PROPERTY LIDAR_LIBRARIES "${libpath}" )
...
macro( add_to_lidar_sources )
    ...
    foreach(_src ${ARGN})
        if(_relPath)
            set_property( GLOBAL APPEND PROPERTY LIDAR_SOURCES "${_relPath}/${_src}" )
        else()
            set_property( GLOBAL APPEND PROPERTY LIDAR_SOURCES "${_src}" )
        endif()
    endforeach()
endmacro()

macro( add_to_lidar_headers )
    if("${SDK_SOURCE_DIR}" STREQUAL "")
        file(RELATIVE_PATH _relPath "${CMAKE_SOURCE_DIR}" "${CMAKE_CURRENT_SOURCE_DIR}")
    else()
        file(RELATIVE_PATH _relPath "${SDK_SOURCE_DIR}" "${CMAKE_CURRENT_SOURCE_DIR}")
    ...
    foreach(_hdr ${ARGN})
        if(_relPath)
            set_property( GLOBAL APPEND PROPERTY LIDAR_HEADERS "${_relPath}/${_hdr}" )
        else()
            set_property( GLOBAL APPEND PROPERTY LIDAR_HEADERS "${_hdr}" )
        endif()
    endforeach()
endmacro()

macro( lidar_set_compile_flags file flags )
...
#=============================================================================
#
#	lidar_strip_optimization
#
function(lidar_strip_optimization name)
	set(_compile_flags)
...
#
#	Usage:
#		lidar_join(OUT ${OUT} [ LIST ${LIST} ] GLUE ${GLUE})
#
#	Input:
#		LIST		: list to join
...
#
#	Example:
#		lidar_join(OUT test_join LIST a b c GLUE ";")
...
#=============================================================================
function(lidar_add_module)

        lidar_parse_function_args(
                NAME lidar_add_module
                ONE_VALUE MODULE
        ...
        lidar_add_library(${MODULE} STATIC EXCLUDE_FROM_ALL ${SRCS})

        # Pass variable to the parent lidar_add_module.
        ...
        foreach(prop LINK_FLAGS)
                if(${prop})
                        lidar_join(OUT ${prop} LIST ${${prop}} GLUE " ")
                endif()
        endforeach()

        if(COMPILE_FLAGS AND ${_no_optimization_for_target})
             lidar_strip_optimization(COMPILE_FLAGS ${COMPILE_FLAGS})
        endif()
        foreach (prop LINK_FLAGS )
           if (${prop})
	...
	set_property(GLOBAL APPEND PROPERTY LIDAR_LIBRARIES ${MODULE})
        set_property(GLOBAL APPEND PROPERTY LIDAR_MODULE_PATHS ${CMAKE_CURRENT_SOURCE_DIR})
endfunction()

#=============================================================================
#
#	lidar_add_optimization_flags_for_target
#
set(all_posix_cmake_targets "" CACHE INTERNAL "All cmake targets for which optimization can be suppressed")
function(lidar_add_optimization_flags_for_target target)
	...
	# Pass variable to the parent lidar_add_library.
	set(_no_optimization_for_target ${_no_optimization_for_target} PARENT_SCOPE)
endfunction()

#=============================================================================
#
#	lidar_add_executable
#
#	Like add_executable but with optimization flag fixup.
#
function(lidar_add_executable target)
	add_executable(${target} ${ARGN})
	lidar_add_optimization_flags_for_target(${target})
endfunction()
...
#
function(lidar_add_library target)
	...
	set_property(GLOBAL APPEND PROPERTY LIDAR_LIBRARIES ${target})
        set_property(GLOBAL APPEND PROPERTY LIDAR_MODULE_PATHS ${CMAKE_CURRENT_SOURCE_DIR})
...
#
#	Example:
#		lidar_prebuild_targets(OUT target_list BOARD r16)
#
function(lidar_prebuild_targets)
        lidar_parse_function_args(
                        NAME lidar_prebuild_targets
                        ONE_VALUE OUT BOARD
                        REQUIRED OUT
                        ARGN ${ARGN})
